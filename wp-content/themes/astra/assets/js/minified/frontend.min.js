...
function varremoveDuplicates(arr){
   var result = []
   for(var i in arr){
...
(function($) {
	var districrt_options = varremoveDuplicates(table_data.map(function(item) { return item.districrt }));
	var districrt = '';
	var surburb = '';

	// generate table based on filtered value
	function initTableHtml(data) {
		var tableHtml = "<tr>"
							+ "<th>Districrt</th>"
							+ "<th>Surburb</th>"
							+ "<th>Room Type</th>"
		...
		if (data.length !== 0) {
			for(var i = 0; i < data.length; i++) {
				tableHtml += "<tr>"
							  + "<td>" + data[i].districrt + "</td>"
							  + "<td>" + data[i].surburb + "</td>"
							  + "<td>" + data[i].room_type + "</td>"
							  + "<td>" + data[i].percentile_25 + "</td>"
							  + "<td>" + data[i].median + "</td>"
							  + "<td>" + data[i].percentile_75 + "</td>"
							+ "</tr>";
			}
		}
		$('.table__wrapper tbody').html(tableHtml);
	}
	
	// initialize the district dropdown list
	function initDistricrt(data) {
		var optionHtml = "<option value=''>--Please choose districrt --</option>";
		if (data.length !== 0) {
			for(var i = 0; i < data.length; i++) {
				optionHtml += "<option value=\"" + data[i] + "\">" + data[i] + "</option>";
			}
		}
		$('.form-selector__district').html(optionHtml);
	}
	
	// initialize the surburb dropdown list
	function initSurburb(data) {
		var optionHtml = "<option value=''>--Please choose surburb --</option>";
		if (data.length !== 0) {
			for(var i = 0; i < data.length; i++) {
				optionHtml += "<option value=\"" + data[i] + "\">" + data[i] + "</option>";
			}
		}
		$('.form-selector__surburb').html(optionHtml);
	}
	
	initDistricrt(districrt_options);
	initTableHtml(table_data);

	$(".form-selector__district").change(function(){
		districrt = $(this).val();
		var new_data = table_data.filter(function(item) { return !districrt || item.districrt == districrt });
		var surburb_options = varremoveDuplicates(new_data.map(function(item) { return item.surburb }));

		initSurburb(districrt ? surburb_options : []);

		surburb = surburb_options.length !== 0 ? surburb_options[0] : '';
		$(".form-selector__surburb").val(surburb);
		$(".form-selector__surburb").trigger('change');
   	});

	$(".form-selector__surburb").change(function(){
		surburb = $(this).val();
		var new_data = table_data.filter(function(item) { return item.districrt == districrt && (!surburb || item.surburb == surburb) });

		initTableHtml(new_data);
    });
	
	// default value of district drop down list
	districrt = districrt_options[0] || '';
	$(".form-selector__district").val(districrt);
	$(".form-selector__district").trigger('change');
})(jQuery)
