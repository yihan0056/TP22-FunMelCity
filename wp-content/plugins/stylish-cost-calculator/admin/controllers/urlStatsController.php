...
class urlStatsController {
	...
	function update( string $url ) {
		if ( in_array( $url, array_keys( $this->urls ) ) ) {
			...
			$this->urls += array( $url => 1 );
		}
		return $this->save( json_encode( $this->urls, JSON_UNESCAPED_UNICODE ) );
	}

	private function save( string $urls ) {
		$query  = $this->db->prepare( "UPDATE `{$this->db->prefix}df_scc_forms` SET `urlStatsArray` =%s WHERE `{$this->db->prefix}df_scc_forms`.`id` = %d", array( $urls, $this->calcId ) );
