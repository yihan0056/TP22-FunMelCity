...
class PageEditTabs extends PagesBreadcrumbs {
	...
	public function __construct() {
		...
		wp_register_script( 'scc-sortable', SCC_URL . 'assets/js/sortable.js', array( 'jquery' ), STYLISH_COST_CALCULATOR_VERSION, true );
		...
		// $sccConfig = ["quoteFormFields" => $formFieldsArray];
		// var_dump($f1);
		...
		// var_dump($f1);
		// var_dump($f1->formstored);
		// var_dump($f1->formstored[0]->value);
		require dirname( __DIR__, 2 ) . '/views/adminHeader.php';
		require dirname( __DIR__, 2 ) . '/views/searchAndTabs.php';
		// require dirname(__DIR__, 1) . "/views/EditTabs.php";
			...
			jQuery(document).ready(function() {
				Swal.fire({
					...
					if (result.isConfirmed) {
						location.href = '<?php echo esc_url( admin_url( 'admin.php?page=Stylish_Cost_Calculator_EditItems' ) ); ?>'
					}
				})
			})
		</script>
	...
	private function updateCalculatorUsageStat() {
		if ( get_option( 'df-scc-save-count' ) ) {
