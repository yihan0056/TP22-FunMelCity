...
$webhookConfigArray      = $isSCCFreeVersion ? array() : json_decode( $form->webhookSettings, true );
//handles error if webhook is not correct
if ( ! is_array( $webhookConfigArray ) ) {
	$webhookConfigArray = array();
}
$webhookQuote      = meks_wp_parse_args(
	$webhookConfigArray[0],
	array(
		'scc_set_webhook_quote' => array(
			'enabled' => false,
			'webhook' => '',
		),
	)
...
$webhookDetailView = meks_wp_parse_args(
	...
	array(
		'scc_set_webhook_detail_view' => array(
			'enabled' => false,
			'webhook' => '',
		),
	)
...
( $translateArray != null || $translateArray != '' ) ? $transletables = getTranslatables( $translateArray ) : $transletables = array();
$sccConfig = array(
	'form_id'                       => $form->id,
...
margin:0px;padding:0px;margin-top:0px;line-height:20px;vertical-align:middle;">
							...
							case '8':
								break;
							?>
									<div class="row df-scc-image-buttons" style="margin-top:20px">
									<?php
									foreach ( $el->elementitems as $item ) :
										$item->opt_default = false;
										?>
						...
						<div class="alert alert-danger" role="alert" style="background:transparent; display: none;"><span class="trn" data-trn-key="Please choose an option">Please choose an option☝️</span> !</div>
						<div style="clear:both"></div>
						<?php
				}
				if ( $el->type == 'Dropdown Menu' ) {
					...
					<?php if ( $applyCustomColumn ) { ?>   
						...
						<?php } else { ?>
							...
							<div id="byjson<?php echo intval( $el->id ); ?>" data-formid="<?php echo intval( $form->id ); ?>" data-elementid="<?php echo intval( $el->id ); ?>"></div>
						<?php } ?>
				...
				if ( $el->type == 'slider' ) {
					...
					foreach ( $el->elementitems as $key => $ei ) {
						...
						if ( in_array( $calculation_type, array( 'default', 'quantity_mod' ) ) ) {
                            if ($key === 0) $coma = "";
                            if ($key > 0) continue;
                        }
					...
					?>
					<?php if ($applyCustomColumn ) { ?>
                    <div id="ssc-elmt-<?php echo intval($el->id) ?>" data-typeElement="<?php echo esc_attr($el->type) ?>" class="clearfix scc_sliderr__ scc-form-field-item <?php echo $style_scc_calculator;
                    if ($hasAccordion) echo ' scc-accordion-panel section_' . $accordion_index;
                    if ($disableFlex) echo " scc-d-block clearfix" ?> slider_in_mobile  scc_font_45">
                    ...
                    <?php } else { ?>
                    <div id="ssc-elmt-<?php echo intval($el->id) ?>" data-typeElement="<?php echo esc_attr($el->type) ?>"  class="clearfix scc_sliderr__ scc-form-field-item <?php echo $style_scc_calculator;
                    echo $hasAccordion ? ' scc-accordion-panel section_' . $accordion_index : ''; ?> slider_in_mobile  scc_font_45">
				...
				<div id="ssc-elmt-<?php echo intval( $el->id ); ?>" class="scc-form-field-item 
					...
					<div class="control-label <?php echo 'scc-col-sm-' . intval( $secondMobileColumnSize ) . ' scc-col-md-' . intval( $secondColumnSize ) . ' scc-col-lg-' . intval( $secondColumnSize ) . ' scc-p-0'; ?> scc_select_opt scc-form-field-item-control" style="padding:0px;">
						<?php if ( $el->value1 == 'default' ) { ?>
							<input id="itemcreateds_0_0_0" onkeyup="triggerSubmit(3, this, <?php echo intval( $el->id ); ?>, 0, <?php echo intval( $form->id ); ?>)" onchange="triggerSubmit(3, this, <?php echo intval( $el->id ); ?>, 0, <?php echo intval( $form->id ); ?>)" class="number_box_text number_box_text-45 itemCreated mandatory_no_45" data-inputtype="number_input" type="number" data-value="1"  <?php if($el->value4 != '') echo 'min='.intval($el->value4) ?> <?php if($el->value3!='') echo 'max='. intval($el->value3) ?>>
						<?php } else { ?>
							<span class="input-number-decrement" onclick="changeNumberQuantity(this,'-',<?php echo intval( $el->id ); ?>,<?php echo intval( $form->id ); ?>)">–</span>
							<input id="itemcreateds_0_0_0" onkeyup="triggerSubmit(3, this, <?php echo intval( $el->id ); ?>, 0, <?php echo intval( $form->id ); ?>)" class="number_box_2_text number_box_text-45 itemCreated mandatory_no_45" data-inputtype="number_input" type="number" data-value="4"  <?php if($el->value4 != '') echo 'min='.intval($el->value4) ?> <?php if($el->value3!='') echo 'max='. intval($el->value3) ?>>
	...
	<div class="scc-total-section" style="padding: 5px 0px 0 5px; font-size:<?php echo esc_attr( $form->titleFontSize ); ?>">
		...
		if ( $form->removeTotal != 'true' ) {
			...
			if ( $form->barstyle == 'scc_tp_style1' ) {
				...
				?>
				">
				<div class="total" style="background-color:<?php //echo esc_attr($form->objectColorPicker) ?> ;color: #fff!important;">
			...
			<?php if ( $form->barstyle == 'scc_tp_style3' || $form->barstyle == null || $form->barstyle == '0' ) { ?>
				<div  style="padding:0px;height:150px">
