...
class SCCQuoteListView {
	...
	public function get_calculator_name() {
		global $wpdb;
		$name = $wpdb->get_results( "SELECT formname FROM `{$wpdb->prefix}df_scc_forms` WHERE id={$this->quotes_from_id}" );
		return $name[0];
	}
	public function __construct() {
		add_action( 'admin_menu', array( $this, 'admin_menu' ), 90 );
		// add_action('wp_ajax_scc_save_quote_management_table_columns_filter', array($this, 'saveQuoteManagementTableColumnsFilter'));
		// add_action('wp_ajax_scc_save_individual_quote_status', array($this, 'saveIndividualQuoteStatus'));
		// add_action('wp_ajax_scc_delete_quote', array($this, 'handleDbQuoteDeletion'));
		add_action(
			'admin_head',
			function () {
		...
		$this->page();
		// add_action('wp_ajax_scc_stripe_action', array( $this,'handleSubmit' ) );
		// add_action('wp_ajax_nopriv_scc_stripe_action', array( $this,'handleSubmit' ) );
	}
	function page() {
		wp_register_style( 'scc-select2', SCC_URL . 'assets/lib/tom-select/tom-select.css', array(), STYLISH_COST_CALCULATOR_VERSION );
		wp_register_script( 'scc-select2', SCC_URL . 'assets/lib/tom-select/tom-select.complete.min.js', array(), STYLISH_COST_CALCULATOR_VERSION );
			...
			<?php if ( true ) : ?>
				...
				<div class="row">
					<div class="col-xs-12 col-md-12 col-lg-12">
						...
						font-size: 15px;letter-spacing: 0.5px;" href="<?php echo esc_url( admin_url( "admin.php?page=Stylish_Cost_Calculator_EditItems&id_form={$_GET["id"]}" ) ); ?> ">Edit Calculator</a>
					</div>
				</div>
			<?php endif ?>
		...
		<script>
			...
			jQuery(document).ready(($) => {
				$('#filter-fields').on('click', ($this) => {
					...
					// $('#filter-fields-options').dialog({
					// 	minWidth: 500,
					// 	open: function () {
					// 		var config = {};
					// 		new TomSelect('.js-example-basic-multiple',config);
					// 	}
					// });
				...
				// pageParameters = jQuery('#page-params').html().trim();
			})
		...
		<?php
		// $this->crm_table->display();
		// $crm_table = new QuoteListTable();
		// $crm_table->prepare_items();
		// $crm_table->display();
	}
	public function get_field_params() {
		...
		$formParams = $wpdb->get_row( "SELECT showFieldsQuoteArray,formFieldsArray FROM {$wpdb->prefix}df_scc_forms WHERE id={$this->quotes_from_id};" );
		...
		// var_dump($formParams->showFieldsQuoteArray);
		/**
		 * !to show in filter
		 * */
		...
		// var_dump($tmp_table_head_to_show);
		// $tmp_table_head_to_show = array_filter($tmp_table_head_to_show, function($value) { return !is_null($value) && $value !== ''; });
	...
	function admin_menu() {
		...
		// add_submenu_page('scc-tabs', 'Stripe Test', 'Stripe Testa', 'manage_options', 'stripe-test', array( $this, 'page' ));
	}
}
...
class QuoteListTable extends \WP_List_Table {
	...
	function prepare_items() {
		...
		$data        = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}df_scc_quote_submissions WHERE {$wpdb->prefix}df_scc_quote_submissions.calc_id={$data_args['form_id']} LIMIT {$data_args['page-max']} OFFSET {$data_args['offset']};" );
		$total_items = $wpdb->get_results( "SELECT COUNT(*) total FROM {$wpdb->prefix}df_scc_quote_submissions WHERE {$wpdb->prefix}df_scc_quote_submissions.calc_id={$data_args['form_id']}" );
	...
	function column_status( $item ) {
		...
		$this->quote_status;
		// reorder the quote status data in a key value format may need this later
		// $quote_status_keyvalue = [];
		// foreach ($this->quote_status as $key => $value) {
		//     $status_key = array_keys($value)[0];
		//     $status_value = array_values($value)[0];
		//     $quote_status_keyvalue[$status_key] = $status_value;
		// }
		// if (isset($quote_status_keyvalue[$currentStatus])) {
		//     // $currentStatus = $quote_status_keyvalue[$currentStatus];
		// }
	...
	function get_columns() {
		$arr1 = array(
			...
			// 'submitted_data'      => __('Raw Data', 'scc'),
		);
