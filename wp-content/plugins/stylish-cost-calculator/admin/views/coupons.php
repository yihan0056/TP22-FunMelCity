...
class Stylish_Cost_Calculator_Coupon {
	...
	public function __construct() {
		...
		wp_localize_script('scc-backend', 'pageCouponSettings', ["nonce" => wp_create_nonce('coupon-settings-page')]);
	...
	function coupon_page() 
	{
        ...
        $translation_array = 'var rt_vars = ' . json_encode(array('rt_adminurl' => admin_url(), 'rt_url' => get_site_url(), 'rt_urlajax' => (get_site_url() . '/wp-admin/admin-ajax.php'), 'a_value' => '10'));
...
<script>
    window.addEventListener('DOMContentLoaded', (event) => {
        ...
        jQuery('.use-tooltip').each(function(index,node) {
            new bootstrap.Tooltip(node, {delay: { "hide": 300 }, trigger: 'hover focus', html: true})
        })
    })
    // COUPONS FUNCTIONS !!
    function saveCoupon() {
