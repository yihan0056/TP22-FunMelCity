...
async function initiateQuoteForm(calcId, callbackFn = null) {
	// TODO make colors, captcha settings, unit column settings available
	...
	if (verifiedMandatoryItems(calcId)) {
		...
		let quoteFormPlaceholder = jQuery('#quote-form-placeholder');
		quoteFormPlaceholder.unbind("hidden.bs.modal").on('hidden.bs.modal', function () {
...
function handleQuoteSubmission(calcId, $this, color = 'cyan', isCaptchaEnabled = false, disableUnitColumn = false, showFormInDetail = false) {
	...
	// [...x.entries()].map(e => e[0])
	// formFieldsArray.map(e => Object.keys(e)[0]).findIndex(e => e == "xtpi1q")
	// [...x.entries()].map(e => formFieldsArray[formFieldsArray.map(ee => Object.keys(ee)[0]).findIndex(eee => eee == e[0])][e[0]])
}
...
function sccMandatoryValidationCheck($this, submitEvent, calculator_id) {
	...
	var scc_mandatory = '';
	jQuery('.mandatory_yes_' + calculator_id).each(function () {
		if (jQuery(this).val() == '' && jQuery(this).is(':visible')) {
			// adding an onetime on change event handler to remove warning if value is truthy
			jQuery(this).one('change', function () {
				if (jQuery(this).val() !== '') {
					jQuery(this).parent().parent().find('.alert-danger').css('display', 'none');
				}
			})
			jQuery(this).parent().parent().find('.alert-danger').html('<span class="trn">Please choose an option☝️</span>');
			jQuery(this).parent().parent().find('.alert-danger').css('display', 'block');
...
function sccProcessCheckout(calcId) {
	console.log(sccData[calcId].stripeCart);
	$fragment_refresh = {
		url: wp.ajax.settings.url,
		type: 'POST',
		data: {
...
function checkCouponCode(calcId) {
	var modalObject = jQuery(event.currentTarget).closest(".df-scc-euiModal");
	modalObject.find(".df-scc-euiButtonContent.df-scc-euiButton__content").html(`<div>
	...
	jQuery.ajax({
		...
		data: {
			...
			coupon_code: modalObject.find("input").val()
		},
		success: function (data, e, i, x) {
			if (typeof data === "object") {
				if (data.data) {
					modalObject.find(".df-scc-euiButtonContent.df-scc-euiButton__content").html('<span class="trn df-scc-euiButton__text">Submit</span>');
					if (data.data === false || data.data === "false") {
						...
						coupon = data.data
						jQuery("#scc-coupon-modal-placeholder").modal("hide")
						// TODO handle coupon calculation
						sccData[calcId].coupon = coupon
						jQuery("select[id*=msdropdown], input[id*=itemcreated]").first().trigger('change')
						return;
						applyCouponDiscount(calcId, coupon)
					}
				}
			} else if (!data) { }
			jQuery("#button_check_coupon").html("Check Coupon Code");
		},
...
function dfsccLoaded(){
	jQuery('[id^=ssc-elmt-]').each(function(){
		...
		function triggerDrop(cntx){
			var el = jQuery(cntx).find('[id^=byjson]')
			let formId = jQuery(el).data('formid')
			let elementId = jQuery(el).data('elementid')
			var json = JSON.parse(jQuery(cntx).find('#json').html())
			let {translation} = sccData[formId].config
			
			let o = translation.find(t => t.key === "Choose an option...")?.translation
			if(!o){
				o = 'Choose an option...'
			}
			var dataArray = [{
			...
			json1.forEach(element => {
				var jsonData = {}
				jsonData.value = element.id
				jsonData.title = element.name.replace(/\\/g,"")
				jsonData.description = element.description.replace(/\\/g,"")
				jsonData.text = element.name.replace(/\\/g,"")
				jsonData.image = element.value1
				dataArray.push(jsonData)
			});
			var jsn = jQuery("#byjson" + elementId).msDropDown({
				byJson: {
					data: dataArray,
					name: 'payments'
				}
			}).data("dd").set('selectedIndex', ss(json1));
			// returns index of item with opt_default
			function ss(json) {
				for (let i = 0; i < json.length; i++) {
					if (json[i].opt_default == 1) {
						return i + 1
					}
				}
			}
			//adds default dropdown to total script
			function pp(json) {
				for (let i = 0; i < json.length; i++) {
					if (json[i].opt_default == 1) {
						triggerSubmit(2, this, 0, 0 + "/" + json[i].id, formId)
					}
				}
			}
			pp(json1)
			 /**
             * *Sends data to Total script 
             * @param previusValue
             * @param selected
             * todo: previusValue is use to replace it in total script with selected
             * ?ids of elements are used to get the price
             */
			var previusValue
			jQuery("#byjson" + elementId).find("select").focus(function() {
				previusValue = this.value
			}).change(function() {
				var selected = this.value
				triggerSubmit(2, this, 0, previusValue + "/" + selected, formId)
				previusValue = this.value
			});
		}
		...
		function triggerSlider(cntx) {
			// console.log(cntx);
			// let el = jQuery(cntx).find('input')
			// let formId = jQuery(el).data('formid')
			// let elementId = jQuery(el).data('elementid')
			// let subid = jQuery(el).data('subid')

			// let itemId = jQuery(this).data('itemid')
			// let optdef = jQuery(this).data('optdefault')
			try {
				// var slider = typeof(elementId) !== 'undefined' ? jQuery("[data-slider-id='slider_itemcreateds_0_0-" + elementId).bootstrapSlider({}) : null;
						...
						for (k = 0; k < rangeMatrix.length; k += 3) {
							...
							if ((parseInt(sliderValue) <= parseInt(rangeMatrix[k + 1]))) {
								const switched = {
									...
									// scc_woo_commerce_product_id: jQuery(ww).attr('scc_woo_commerce_product_id'),
									calculator_id: 2
								};
								/**
...
function triggerSubmit(type, dom, element, item, calcId) {
	 ...
	 var as = container.querySelectorAll("[id*=ssc-elmt-]").forEach(function(e) {
		...
		if (chck) {
			var ch = e.querySelectorAll("input").forEach(function (e) {
				...
				if (s) {
					...
					if (index < 0) {
						// TODO: handle variable math
						// sccUtils.handleVariableMath(calcId, {id: parseInt(i[i.length - 1]), type: 'checkbox'})
		...
		if(slider){
			...
			if (!index) {
				...
				// let scale = jQuery(this).find('.scc-form-slider-field > input').attr('data-calculation-type') == "sliding" ? "true" : "false"
	...
	// let {tseparator} = sccData[calcId].config
	// if(tseparator==="comma"){
	//     total.text(priceCommaStyler(parseFloat(totalTotal[calcId]).toFixed(2)))
	// }else{
	//     total.text(parseFloat(totalTotal[calcId]).toLocaleString(navigator.languages[0]))
	// }
	...
	// ccx = sections.map(q => q.subsection.map(e => ({name: q.name, elements: e.element})))
	// ccx.map(e => ({n*9ame: e[0].name, elements: e.map(c => c.elements)})).map(eq => ({name: eq.name, els: eq.elements.flat()}))
	// itemsOrdered = sections
	//     .map((q) => q.subsection.map((e) => e.element))
	//     .map((eq) => eq.flat())
	//     .flat();
...
function verifiedMandatoryItems(calcId) {
	var container = document.querySelector("#scc_form_"+ calcId)
	container.querySelectorAll(".alert-danger").forEach(function(e) {
	...
	for (let i = 0; i < oppo[calcId].length; i++) {
		if(document.querySelector("#ssc-elmt-" + oppo[calcId][i]).querySelector(".alert-danger"))
			document.querySelector("#ssc-elmt-" + oppo[calcId][i]).querySelector(".alert-danger").style.display = "block"
			var oo = document.querySelector("#ssc-elmt-" + oppo[calcId][i]).nextElementSibling
			if(oo.classList.contains('alert-danger')) oo.style.display = "block"
		if (i == 0) {
...
// currencyStyleOption = 0
// currencyLabel = 'PEN'
/**
 * *Fixes margin between calculators
 * ?add margin to calculators except the first 
 */
jQuery(document).ready(function () {
	// jQuery("[id^=scc_form_]").slice(1).css("margin-top","100px")
})
