...
( function() {
		...
		const container = div({
			children: [
				...
				getNewApplicationOption(
					...
					/* %s: conditional "and views" text. */
					__( 'Select existing forms%s and add them to the application', 'formidable-pro' ).replace( '%s', getConditionalAndViewsText() ),
	...
	function getViewsUpsell() {
		return div({
			...
			children: [
				...
				div({
					children: [
						document.createTextNode( __( 'Build custom layouts in a visual drag and drop editor, and present data received with Formidable Forms.', 'formidable-pro' ) ),
						a({
	...
	function getNewApplicationOption( title, description, icon, clickHandler ) {
		const option = div({
			className: 'frm-new-application-option',
			children: [
				span({ className: 'frm-new-application-option-icon-wrapper', child: icon }),
				div({
					children: [
						tag( 'h3', { text: title }),
