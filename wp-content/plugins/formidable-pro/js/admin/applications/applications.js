( function() {
	...
	function removeLockIconFromCard( card ) {
		...
		const svg = cardTitle.previousElementSibling;
		if ( ! svg || 'svg' !== cardTitle.previousElementSibling.nodeName.toLowerCase() ) {
			return;
		}
	...
	function getCustomApplicationCard( data ) {
		...
		if ( frmProApplicationsVars.canAddApplications ) {
			...
			card.addEventListener( 'click', event => event.target !== deleteTrigger && card.querySelector( 'span a' ).click() );
